...
9/27/17 = 34586 sequences downloaded (some added to selected list)
Next update, remove restriction on PREDICTED genes (all Hyalella, so not vital right now, but currently using on in selected list so shouln't exclude them - only about 57 total as of 9/28/17)
#### (Amphipoda[ORGN] or Isopoda[ORGN]) ((coi or co1 or coxi or cox1 or "cytochrome oxidase subunit 1" or "cytochrome oxidase subunit i" or "cytochrome oxidase i" or "cytochrome c oxidase subunit 1" or "cytochrome c oxidase subunit i") or (("large subunit" mitochondrial) or 16S) or (("small subunit" not mitochondrial) or 18S) or 28S or (H3)) NOT (Environmental samples[ORGN] or genome[Title] or "internal transcribed spacer" or txid555092[Organism:exp] or unclassified)
...
##Output all Leucothoe COI sequences

```{r, eval=FALSE, tidy=TRUE}
library(stringr)
#separate out Leu COI sequences:

#df_Leu=df[which(str_detect(df$species_name,"Leucothoe")),]

Leu_list=as.logical(str_detect(df$species_name,"Leucothoe") + str_detect(df$species_name,"Anamixis") + str_detect(df$species_name,"Paranamixis"))
df_Leu=df_Leu=df[which(Leu_list),]
df_Leu_COI=df_Leu[which(df_Leu$gene=="mt_COI "),]

#append author name to accession#
df_Leu_COI$accession=str_c(df_Leu_COI$accession,substr(df_Leu_COI$author_ref1,1,str_locate(df_Leu_COI$author_ref1,",")-1),sep="|")
#create fasta file
dataname="Leu_COI_all"
create.fasta(dataname,df_Leu_COI)

#separate out Leu 18S sequences:
df_Leu_18S=df_Leu[which(df_Leu$gene=="n_18S "),]
#create fasta file
dataname="Leu_18S_all"
create.fasta(dataname,df_Leu_18S)
```